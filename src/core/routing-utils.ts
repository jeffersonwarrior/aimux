 ...
 */
export function analyzeRequestRequirements(request: AIRequest): RequestRequirements {
  ...
  if (hasThinking) capabilities.push('thinking');
  if (hasVision) capabilities.push('vision');
  if (hasTools) capabilities.push('tools');
  if (needsStreaming) capabilities.push('streaming');
  if (request.messages.some(msg => msg.role === 'system')) capabilities.push('systemMessages');
  if (request.temperature !== undefined) capabilities.push('temperature');
  if (request.top_p !== undefined) capabilities.push('topP');
 ...
 */
export function hasThinkingRequest(request: AIRequest): boolean {
  ...
  return (
    thinkingIndicators.some(indicator => lowerText.includes(indicator)) ||
 ...
 */
export function hasVisionRequest(request: AIRequest): boolean {
  // Check explicit metadata first
 ...
 */
export function hasToolsRequest(request: AIRequest): boolean {
  // Check explicit metadata first
...
export function determineRequestType(
  ...
  hasVision: boolean,
  hasTools: boolean
): 'thinking' | 'regular' | 'vision' | 'tools' | 'hybrid' {
 ...
 */
export function estimateRequestComplexity(request: AIRequest): 'low' | 'medium' | 'high' {
  const textContent = extractTextContent(request);
  ...
  // Token count factor
  if (estimatedTokens > 8000) complexityScore += 3;
  else if (estimatedTokens > 4000) complexityScore += 2;
  else if (estimatedTokens > 2000) complexityScore += 1;

  // Content complexity factor
  const lowerText = textContent.toLowerCase();
  if (hasCodeContent(lowerText)) complexityScore += 1;
  if (hasMathematicalContent(lowerText)) complexityScore += 1;
  if (hasComplexProblemIndicators(lowerText)) complexityScore += 2;

  // Tools/complex parameters factor
  if (request.tools && request.tools.length > 3) complexityScore += 2;
  else if (request.tools && request.tools.length > 0) complexityScore += 1;
 ...
 */
export function estimateTokenUsage(request: AIRequest): number {
  let tokens = 0;
    ...
    if (typeof message.content === 'string') {
      ...
      for (const part of message.content) {
        if (part.type === 'text' && part.text) {
          ...
          if (detail === 'high') tokens += 85;
          else if (detail === 'low') tokens += 65;
          else tokens += 85; // Default to high detail
        }
      }
    }
 ...
 */
export function determineRequestPriority(
  request: AIRequest,
  requestType: string,
  complexity: 'low' | 'medium' | 'high'
): 'low' | 'medium' | 'high' {
 ...
 */
export function extractTextContent(request: AIRequest): string {
  let textContent = '';
 ...
 */
function hasComplexProblemIndicators(text: string): boolean {
 ...
 */
function hasMathematicalContent(text: string): boolean {
...
export function normalizeCapabilityRequirements(
  ...
  const capabilityPriority: (keyof ProviderCapabilities)[] = [
    'thinking',
    'vision',
    'tools',
    'streaming',
 ...
 */
export function canProviderSatisfyRequirements(
 ...
 */
export function calculateProviderCompatibility(
  provider: any, // BaseProvider instance
  requirements: RequestRequirements
): number {
  let score = 0;
 ...
 */
export function determineLoadBalancingStrategy(
  recentRequests: RequestRequirements[]
): 'round-robin' | 'weighted' | 'least-connections' | 'performance-based' {
