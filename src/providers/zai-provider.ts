...
export class ZAIProvider extends BaseProvider {
  public readonly name = 'Z.AI';
  public readonly id = 'z-ai';
  ...
  private readonly DEFAULT_BASE_URL = 'https://api.z.ai/api/anthropic/v1';
  ...
  protected handleError(error: any): ProviderError {
    if (error.response) {
      ...
      return new ProviderError(`Z.AI API error: ${message}`, this.name, statusCode, error);
    } else if (error.code === 'ECONNABORTED') {
      // Timeout error
      return new ProviderError('Z.AI request timeout', this.name, 408, error);
    } else if (error.code === 'ENOTFOUND' || error.code === 'ECONNREFUSED') {
      // Network error
      return new ProviderError('Z.AI network error', this.name, 503, error);
    } else {
      // Other errors
      return new ProviderError(`Z.AI provider error: ${error.message}`, this.name, 500, error);
    }
  }
