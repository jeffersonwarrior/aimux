...
import { SimpleRouter, SimpleRouterConfig } from './simple-router';
...
export class RouterManager extends EventEmitter {
  ...
  private config: RouterManagerConfig;
  ...
  private errors: Array<{ timestamp: Date; message: string; provider?: string }> = [];
  ...
  async initialize(): Promise<void> {
    try {
      logger.info('Initializing router manager...');

      // Create the router instance
      this.router = new SimpleRouter(this.providerRegistry, this.routingEngine, this.config.router);
      ...
      logger.info('Router manager initialized successfully');
    } catch (error) {
      logger.error('Failed to initialize router manager:', error);
      throw error;
    }
  }
   ...
   */
  async updateProvider(providerConfig: ProviderConfig): Promise<void> {
    try {
      logger.info(`Updating provider configuration: ${providerConfig.name}`);
      ...
      logger.info(`Provider ${providerConfig.name} updated successfully`);
    } catch (error) {
      logger.error(`Failed to update provider ${providerConfig.name}:`, error);
      this.recordError(
        `Failed to update provider ${providerConfig.name}`,
        error instanceof Error ? error.message : 'Unknown error',
        providerConfig.name
      );
      throw error;
  ...
  async removeProvider(providerName: string): Promise<void> {
    try {
      logger.info(`Removing provider: ${providerName}`);
      ...
      logger.info(`Provider ${providerName} removed successfully`);
    } catch (error) {
  ...
  getStatus(): RouterStatus {
    ...
    return {
      ...
      config: routerStatus?.config || ({} as SimpleRouterConfig),
  ...
  getMetrics(): RouterMetrics {
    ...
    return {
      ...
      uptime,
      averageResponseTime:
        this.metrics.requestsHandled > 0
  ...
  async healthCheck(): Promise<{
    ...
    results.overall =
      results.router && Object.values(results.providers).some((p: any) => p.status === 'healthy');
  ...
  private setupErrorHandling(): void {
    ...
    if (typeof (this.providerRegistry as any).on === 'function') {
      (this.providerRegistry as any).on('provider:error', (data: any) => {
        this.recordError('Provider error', data.error, data.provider);
      });
    }

    // Listen to routing engine events (if engine supports events)
    if (typeof (this.routingEngine as any).on === 'function') {
      (this.routingEngine as any).on('routing:error', (data: any) => {
  ...
  private scheduleRestart(): void {
    if (this.restartAttempts >= (this.config.maxRestartAttempts || 10)) {
      logger.error('Max restart attempts reached. Stopping automatic restarts.');
    ...
    logger.info(
      `Scheduling router restart attempt ${this.restartAttempts}/${this.config.maxRestartAttempts} in ${delay}ms`
    );
   ...
   * Record an error
   */
  private recordError(message: string, details: string, provider?: string): void {
    const error = {
      timestamp: new Date(),
    ...
    logger.error('Router error recorded:', error);
  }
  ...
  updateRequestMetrics(providerName: string, success: boolean, responseTime: number): void {
    ...
    const totalResponseTime =
      providerMetrics.responseTime * (providerMetrics.requests - 1) + responseTime;
  ...
  async cleanup(): Promise<void> {
    try {
      ...
      logger.info('Router manager cleaned up successfully');
    } catch (error) {
      logger.error('Failed to cleanup router manager:', error);
      throw error;
    }
  }
}
