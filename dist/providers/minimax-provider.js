...
exports.MinimaxM2Provider = void 0;
 ...
 */
class MinimaxM2Provider extends base_provider_1.BaseProvider {
    name = 'Minimax M2';
    id = 'minimax-m2';
    capabilities = {
        thinking: true,
    ...
    async makeRequest(request) {
        ...
        try {
            ...
            const response = await this.httpClient.post(`${this.config?.baseUrl || this.DEFAULT_BASE_URL}/messages`, minimaxRequest, {
    ...
    handleError(error) {
        if (error.response) {
            ...
            return new types_1.ProviderError(`Minimax M2 API error: ${message}`, this.name, statusCode, error);
        }
        else if (error.code === 'ECONNABORTED') {
            // Timeout error
            return new types_1.ProviderError('Minimax M2 request timeout', this.name, 408, error);
        }
        else if (error.code === 'ENOTFOUND' || error.code === 'ECONNREFUSED') {
            // Network error
            return new types_1.ProviderError('Minimax M2 network error', this.name, 503, error);
        }
        else {
            // Other errors
            return new types_1.ProviderError(`Minimax M2 provider error: ${error.message}`, this.name, 500, error);
        }
    }
    /**
...
exports.MinimaxM2Provider = MinimaxM2Provider;
//# sourceMappingURL=minimax-provider.js.map