...
exports.SyntheticProvider = void 0;
 ...
 */
class SyntheticProvider extends base_provider_1.BaseProvider {
    name = 'Synthetic.New';
    id = 'synthetic-new';
    capabilities = {
        thinking: true,
    ...
    DEFAULT_ANTHROPIC_URL = 'https://api.synthetic.new/anthropic';
    ...
    async authenticate(apiKey) {
        try {
            ...
            const response = await this.httpClient.get(`${this.config?.baseUrl || this.DEFAULT_MODELS_URL}`, {
                headers: {
    ...
    async fetchModels(forceRefresh = false) {
        ...
        try {
            ...
            if (response.status !== 200) {
                throw new types_1.ProviderError(`Failed to fetch models: HTTP ${response.status}`, this.name, response.status);
            }
    ...
    async makeRequest(request) {
        ...
        try {
            ...
            const response = await this.httpClient.post(endpoint, syntheticRequest, {
    ...
    normalizeModels(models) {
        return models.map((model) => ({
            id: model.id,
            name: model.id?.replace(/-/g, ' ').replace(/\b\w/g, (l) => l.toUpperCase()) || model.id,
            ...
            context_length: model.context_length || (model.id?.includes('claude-3-opus') ? 200000 : 100000),
    ...
    handleError(error) {
        if (error.response) {
            ...
            return new types_1.ProviderError(`Synthetic.New API error: ${message}`, this.name, statusCode, error);
        }
        else if (error.code === 'ECONNABORTED') {
            // Timeout error
            return new types_1.ProviderError('Synthetic.New request timeout', this.name, 408, error);
        }
        else if (error.code === 'ENOTFOUND' || error.code === 'ECONNREFUSED') {
            // Network error
            return new types_1.ProviderError('Synthetic.New network error', this.name, 503, error);
        }
        else {
            // Other errors
            return new types_1.ProviderError(`Synthetic.New provider error: ${error.message}`, this.name, 500, error);
        }
    }
    /**
...
exports.SyntheticProvider = SyntheticProvider;
//# sourceMappingURL=synthetic-provider.js.map